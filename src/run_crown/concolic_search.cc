...
void Search::RunProgram(const vector<Value_t>& inputs, SymbolicExecution* ex) {
	...
	if (!TCdir.empty())
	{
	    ...
	    /* Kunwoo Park (2018-11-09)
             * If the file generated by SYM_assume() exists 
             * (i.e., current test input does not satisfy the assumption),
             * 1. delete the corresponding test input file
             * 2. delete the file generated by SYM_assume()
             * 3. reduce the number of iteration (i.e., num_iters_) by 1                      
             * 
	     * CONSIDERATION
             * When running "run_crown" with SYM_assume() used in the target program, 
             * the number of iterations printed on the terminal may be duplicated.
	     * This is because the value of "num_iters_" may not change (+1 and then -1) */
	    FILE *fp = fopen("unsatisfy", "r");
	    if (fp) {
		char command[100];
		strcpy(command, "rm ");
